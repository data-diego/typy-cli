...
pub fn run(
    ...
    // First-round inputs only; Tab restart drops them so the next round is a fresh test.
    let mut round_repeat_words = repeat_words;
    let mut round_ghost_data = ghost_data;
    ...
    let action = 'session: loop {
        ...
        let is_repeat = round_repeat_words.is_some();

        let mut game = if let Some(words) = round_repeat_words.take() {
            Game::new(words)
        } else {
            let mut g = Game::new(
        ...
        let ghost_data = round_ghost_data.take();
        ...
        // Clear before each round so a Tab restart wipes the previous round in place
        stdout.execute(Clear(ClearType::All))?;
        for (i, words) in game.list.iter().enumerate() {
            print_words(line_xs[i], y + i as u16, words, &stdout, &theme)?;
        }
        ...
        let remaining_time = Arc::new(Mutex::new(duration));
        ...
        let timer_thread = thread::spawn(move || {
            ...
            if let Err(e) = start_timer(duration, timer_expired_clone, remaining_time_clone) {
        ...
        // Stop the round's timer thread regardless of how the inner loop exited
        timer_expired.store(true, Ordering::Relaxed);
        timer_thread
            .join()
        ...
        if restart_requested {
            // Tab pressed mid-game: restart in place without leaving the alternate screen
            continue 'session;
        }

        let round_action = if !game.quit {
